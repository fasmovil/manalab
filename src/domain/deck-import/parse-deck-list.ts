...
export function parseDeckList(rawText: string): ParseResult {
  ...
  for (const [offset, sourceLine] of rawText.split(/\r?\n/).entries()) {
    ...
    if (/^(#|\/\/|;)/.test(trimmed)) { const entry: SourceEntry = { lineNumber, rawText: sourceLine, commanderDesignated: false, inlineCategoryMetadata: [] }; entries.push(entry); problems.push({ sourceEntry: entry, category: 'unsupported-syntax', explanation: 'Comments are not card entries.' }); continue; }
    ...
    if (quantity) { const parsedQuantity = Number(quantity[1]); if (!Number.isSafeInteger(parsedQuantity) || parsedQuantity < 1) { problems.push({ sourceEntry: entry, category: 'invalid-quantity', explanation: 'Quantity must be a positive whole number.' }); entries.push(entry); continue; } entry.parsedQuantity = parsedQuantity; entry.suppliedName = quantity[2].trim(); }
    else if (/^[-+]?\d+(?:\.\d+)?\s*[xX]?(?:\s|$)/.test(body)) { problems.push({ sourceEntry: entry, category: 'invalid-quantity', explanation: 'Quantity must be a positive whole number.' }); entries.push(entry); continue; }
    else entry.parsedQuantity = 1, entry.suppliedName = body;
    if (entry.parsedQuantity !== undefined && entry.parsedQuantity < 1) problems.push({ sourceEntry: entry, category: 'invalid-quantity', explanation: 'Quantity must be a positive whole number.' });
    else if (!entry.suppliedName) problems.push({ sourceEntry: entry, category: 'malformed-entry', explanation: 'A card entry requires a card name and positive quantity.' });
    entries.push(entry);
  }
